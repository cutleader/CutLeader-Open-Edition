...
#define TLC_SHOWSELFULLROWS        0x00000080  // show selected item in full row mode

#define TLC_HEADER                 0x00000100  // show header
#define TLC_HGRID                  0x00000200  // show horizontal grid lines
#define TLC_VGRID                  0x00000400  // show vertical grid lines
#define TLC_TGRID                  0x00000800  // show tree horizontal grid lines ( when HGRID & VGRID )

#define TLC_HGRID_EXT              0x00001000  // show extension horizontal grid lines
#define TLC_VGRID_EXT              0x00002000  // show extension vertical grid lines
#define TLC_HGRID_FULL             0x00004000  // show full horizontal grid lines
...
#define TLC_NOAUTOCHECK            0x40000000  // do NOT auto set check box of parent & child
#define TLC_NOAUTOLOCK             0x80000000  // do NOT auto set lock box of parent & child
...
#define TLB_UNKNOW                 0x0004      // unknown button

// TreeListCtrl HitTest Position
#define TLHT_ONITEMSPACE           0x0001      // on space
#define TLHT_ONITEMBUTTON          0x0002      // on button
#define TLHT_ONITEMCHECKBOX        0x0004      // on check box
#define TLHT_ONITEMLOCKBOX         0x0008      // on lock box
...
// TreeListCtrl Next Item Const
#define TLNI_ABOVE                 0x0100      // find next item form down to up
#define TLNI_BELOW                 0x0200      // find next item form up to down

// TreeListCtrl SetColumnWidth Const
#define TLSCW_AUTOSIZE             -1          // auto size column width by item width
#define TLSCW_AUTOSIZE_USEHEADER   -2          // auto size column width by item width and header width
#define TLSCW_AUTOSIZEV            -3          // auto size column width by visible item width
#define TLSCW_AUTOSIZEV_USEHEADER  -4          // auto size column width by visible item width and header width
...
// notes:
//   1) in this tree list, all cells in a column have the same
//      behavior, in some case, we should change this behavior.
//      so we can wrapper this class.
//   2) if an exe module will use this tree list, it will add the 
//      bmp and icon resource which will be used in this tree list into its resource.

///////////////////////////////////////////////////////////////////////////////

class ClUILib_Export CTreeListCtrl : public CWnd
{
...
public:
  ...
  /************************************************************************/  
  // we should provide cursor and bmp which will be used by this tree list.
  // notes:
  //   1) these functions will be called after "CTreeListCtrl()".

  void InitTreeCurBMP(int iButtonCur, int iCheckCur, int iLockCur, int iDropCur, 
				      int iStopCur, int iBackgdBMP, int iDisableBMP, 
				      int iCheckBMP, int iLockBMP, int iTreeBMP);

  void InitHeadCurBMP(int iSizeCur, int iSplitCur, int iSortBMP, int iHeadBMP);
  /************************************************************************/
...
public:
  ...
  // notes: classed which inherit from "CTreeListCtrl" can overwrite
  //        this function.
  virtual BOOL   BeginModify( CTreeListItem* pItem, int iCol );

  virtual BOOL   UpdateModify();
  virtual BOOL   FinishModify();
  BOOL           CancelModify();
