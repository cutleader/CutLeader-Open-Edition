...
CutSequenceOrderChangeActionB::CutSequenceOrderChangeActionB(PartLayoutDataPtr pPartLayoutData, SequenceDataPtr pSequenceData, 
...
BOOL CutSequenceOrderChangeActionB::LButtonDown(double& dPtX, double& dPtY)
{
	...
	if (m_pCurrentCutSequences->size() > 0)
	{
		...
		else
		{
			...
			// 执行命令。
			ICommandPtr pCommand(new SequenceLocationMoveCommand(GetEditorData(), m_pSequenceData->GetCutSequences(), m_pConfigData, m_pCurrentCutSequences,
				m_pBaseCutSequence, m_pPartLayoutData->GetPartInstances()));
			pCommand->Do();
